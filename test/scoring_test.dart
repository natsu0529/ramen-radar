...
void main() {
  ...
  group('computeRanking', () {
    test('sorts by score desc then distance asc', () {
      const p1 = Place(id: '1', name: 'A', rating: 4.5, location: LatLng(0, 0));
      const p2 = Place(id: '2', name: 'B', rating: 4.5, location: LatLng(0, 0));
      final cands = [
        const Candidate(place: p1, distanceKm: 1.23), // rounded 1.2 => score 7.8
        const Candidate(place: p2, distanceKm: 1.28), // rounded 1.3 => score 7.7
      ];
      final rank = computeRanking(cands);
      expect(rank.first.place.id, '1');

      // tie-breaker: same score, closer distance first
      const p3 = Place(id: '3', name: 'C', rating: 4.5, location: LatLng(0, 0));
      const p4 = Place(id: '4', name: 'D', rating: 4.5, location: LatLng(0, 0));
      final tie = [
        const Candidate(place: p3, distanceKm: 1.21), // rounded 1.2 => 7.8
        const Candidate(place: p4, distanceKm: 1.24), // rounded 1.2 => 7.8
      ];
  ...
  group('computeSpotGrade', () {
    test('maps average to grades', () {
      RankingEntry e(double s) => RankingEntry(place: const Place(id: 'x', name: 'X', rating: 4, location: LatLng(0, 0)), score: s, roundedDistanceKm: 0);
